...
public class ChatController(OpenAiChatClient llm, OpenAiEmbeddingClient embedder, QdrantClient qdrant) : ControllerBase
{
    ...
    public async Task<IActionResult> Message([FromBody] ChatRequest request)
    {
        ...
        const string systemPrompt = """
                                    ...
                                    1. ГРАНИЦЫ ЗНАНИЙ: ты знаешь ТОЛЬКО про УрФУ, НИУ ВШЭ, МГУ, СпбГУ, ИТМО. Если пользователь спрашивает про другие вузы (например, Политех, МФТИ и др.), сразу отвечай: 'Я могу проконсультировать вас только по вопросам, связанным с УрФУ, НИУ ВШЭ, СПбГУ, ИТМО и МГУ.'
                                    ...
                                    4. ОТСУТСТВИЕ ДАННЫХ: если запрос конкретный, но в CONTEXT нет информации, честно скажи: 'К сожалению, я не нашел этой информации в официальных документах.' Не придумывай от себя.
