...
public class ChatController(OpenAiChatClient llm, OpenAiEmbeddingClient embedder, QdrantClient qdrant) : ControllerBase
{
    ...
    public async Task<IActionResult> Message([FromBody] ChatRequest request)
    {
        ...
        var historyText = request.History is { Count: > 0 } 
            ...
            : "";
        var rewritePrompt = "Ты - AI, который превращает неточные вопросы пользователей в идеальные поисковые запросы. " +
                            "ОБЯЗАТЕЛЬНО используй КОНТЕКСТ ПРЕДЫДУЩЕГО ДИАЛОГА, чтобы понимать суть.\n" +
                            "ПРАВИЛА:\n" +
                            "1. Если это просто приветствие или бессмыслица, верни ровно одно слово: 'CHITCHAT'.\n" +
                            "2. ВАЖНО: Если пользователь спрашивает про ЛЮБОЙ вуз, кроме [УрФУ, МГУ, НИУ ВШЭ, ИТМО, СПбГУ] (например, НГУ, МФТИ, КФУ, РУДН и т.д.) — верни ровно одно слово: 'OUT_OF_SCOPE'.\n" +
                            "3. В остальных случаях исправляй опечатки, раскрывай аббревиатуры и выводи ТОЛЬКО переписанный запрос без кавычек.\n\n" +
                            historyText;
        ...
        const string systemPrompt = """
                                    ...
                                    3. ЗАПРЕТ НАВЯЗЫВАНИЯ: НИКОГДА не предлагай альтернативные вузы или направления по своей инициативе. Если пользователь спрашивает про ВШЭ, говори ТОЛЬКО про ВШЭ.
